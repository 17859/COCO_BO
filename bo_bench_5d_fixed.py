...
# 1. Load Dataset (D_0)
# =====================================================
def load_initial_dataset(csv_path='D:/Users/TingYuLin/Desktop/py12/chemical_sparse_data_f108_20260115_1.csv', n_init=None, seed=None):
    """
...
def optimize_acquisition_simplex(acquisition_fn, train_X=None, train_Y=None, n_candidates=2000, seed=42):
    ...
    if train_X is not None and train_Y is not None and len(train_X) > 0:
        ...
        try:
            ...
            for r in range(1, num_restarts):
                k = np.random.randint(2, 6)  # k ~ 2 to 5 for D=5
    ...
    try:
        ...
        for b in range(n_batches):
            ks = np.random.randint(2, 6, size=batch_size)
...
def run_bo_experiment(csv_path, n_init=100, n_iter=50, seed=42):
    ...
    best_initial_true = np.min(y_init_true)
    initial_log_regret = np.log10(max(best_initial_true - true_optimum, 1e-10))
    ...
    for _ in range(n_test):
        k = np.random.randint(2, 6)  # k ~ 2 to 5 for D=5
        idx = np.random.choice(5, k, replace=False)
        vals = np.random.dirichlet(np.ones(k))
        x = np.zeros(5); x[idx] = vals
...
if __name__ == "__main__":
    CSV_FILE = 'D:/Users/TingYuLin/Desktop/py12/chemical_sparse_data_f108_20260115_1.csv'
    try:
